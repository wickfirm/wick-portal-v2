...
export default function ProjectResources({
  ...
  async function addResource(e: React.FormEvent) {
    ...
    const res = await fetch("/api/project-resources", {
      ...
      body: JSON.stringify({ ...newResource, projectId }),
    });
  ...
  async function deleteResource(id: string) {
    ...
    const res = await fetch("/api/project-resources/" + id, { method: "DELETE" });
    if (res.ok) {
  ...
  function guessTypeFromUrl(url: string): string {
    ...
    if (url.includes("staging") || url.includes("dev.") || url.includes("test.")) return "STAGING";
    return "OTHER";
  }
    ...
    <div style={{ 
        ...
        <form onSubmit={addResource} style={{ padding: 20, borderBottom: "1px solid " + theme.colors.borderLight, background: theme.colors.bgPrimary }}>
          <div style={{ display: "grid", gap: 12, marginBottom: 12 }}>
            <input
              ...
              placeholder="Resource name (e.g., Brand Assets Folder)"
              style={inputStyle}
              autoFocus
            />
            <input
      ...
      {resources.length === 0 ? (
        <div style={{ padding: 32, textAlign: "center", color: theme.colors.textMuted, fontSize: 14 }}>
          No resources yet. Add links to important files and tools.
        </div>
      ) : (
        <div>
